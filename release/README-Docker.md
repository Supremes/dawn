# Fatcat Docker Setup

这个目录包含了 Fatcat 博客平台的 Docker Compose 配置文件和相关工具。

## 📁 目录结构

```
release/
├── docker-compose.yaml          # 主要的 Docker Compose 配置
├── .env                        # 环境变量配置文件
├── Dockerfile                  # 应用容器构建文件
├── dawn.sql                  # 数据库初始化脚本
├── vue/                        # 前端构建文件
├── config/                     # 配置文件目录
│   ├── mysql/
│   │   └── my.cnf             # MySQL 配置文件
│   ├── nginx/
│   │   └── nginx.conf         # Nginx 配置文件
│   └── elasticsearch/
│       └── elasticsearch.yml   # Elasticsearch 配置文件
├── scripts/                    # 管理脚本目录
│   ├── manage.sh              # 项目管理主脚本
│   ├── backup.sh              # 数据备份脚本
│   ├── restore.sh             # 数据恢复脚本
│   ├── generate-ssl.sh        # SSL证书生成脚本
│   ├── fatcat-start.sh        # 启动脚本
│   ├── replace-vue.sh         # Vue文件替换脚本
│   └── restart.sh             # 重启脚本
├── ssl/                        # SSL证书目录
├── logs/                       # 日志文件目录
├── backups/                    # 备份文件目录（运行时创建）
└── tmp-files/                  # 临时文件目录
```

## 🚀 快速开始

### 1. 环境准备

确保您已安装：
- Docker (>= 20.0)
- Docker Compose (>= 2.0)

### 2. 配置环境变量

```bash
# 环境变量已经是正式的 .env 文件，根据需要修改配置
vim .env
```

### 3. 启动服务

```bash
# 使用管理脚本启动（推荐）
./scripts/manage.sh start

# 或者直接使用 docker-compose
docker-compose up -d
```

### 4. 检查服务状态

```bash
./scripts/manage.sh status
```

## 🔧 管理脚本使用

### 主管理脚本 (manage.sh)

```bash
# 查看帮助
./scripts/manage.sh help

# 启动所有服务
./scripts/manage.sh start

# 停止所有服务
./scripts/manage.sh stop

# 重启服务
./scripts/manage.sh restart

# 查看服务状态
./scripts/manage.sh status

# 查看日志（所有服务）
./scripts/manage.sh logs

# 查看特定服务日志
./scripts/manage.sh logs fatcat

# 构建应用镜像
./scripts/manage.sh build

# 清理资源
./scripts/manage.sh clean

# 重置所有数据（危险操作）
./scripts/manage.sh reset
```

### 备份和恢复

```bash
# 创建数据备份
./scripts/backup.sh

# 查看可用备份
./scripts/restore.sh

# 恢复数据库
./scripts/restore.sh mysql_20240715_143000.sql.gz
```

### SSL证书生成

```bash
# 生成自签名SSL证书
./scripts/generate-ssl.sh
```

## 🏗️ 架构组件

### 数据库层
- **MySQL 8.0**: 主数据库，存储用户数据、文章、评论等
- **Redis 7.2**: 缓存层，提升性能

### 中间件层
- **RabbitMQ 3.12**: 消息队列，处理异步任务
- **Elasticsearch 7.17**: 搜索引擎，提供全文搜索功能
- **MinIO**: 对象存储，管理文件上传

### 应用层
- **Fatcat App**: Spring Boot 应用，提供 API 和管理界面
- **Nginx**: 反向代理和静态文件服务器

## 🌐 服务访问

启动服务后，您可以通过以下地址访问：

- **博客前台**: http://localhost
- **管理后台**: http://localhost:8888
- **应用 API**: http://localhost:8080
- **管理 API**: http://localhost:8081
- **RabbitMQ 管理**: http://localhost:15672 (guest/guest)
- **MinIO 控制台**: http://localhost:9001 (minioadmin/minioadmin)
- **Elasticsearch**: http://localhost:9200

## 📊 数据持久化

所有服务数据都存储在 Docker 命名卷中：

- `mysql_data` - 数据库文件
- `redis_data` - 缓存数据
- `elasticsearch_data` - 搜索索引
- `minio_data` - 对象存储
- `fatcat_logs` - 应用日志
- `fatcat_uploads` - 用户上传文件

## 🔒 安全配置

### 生产环境建议

1. **修改默认密码**: 更新 `.env` 文件中的所有密码
2. **启用 HTTPS**: 使用 `generate-ssl.sh` 生成证书或配置正式证书
3. **网络安全**: 配置防火墙规则，只开放必要端口
4. **定期备份**: 设置定时任务运行 `backup.sh`
5. **监控日志**: 定期检查应用和系统日志

### 开发环境

当前配置适用于开发环境，包含了便于调试的设置：
- 调试端口开放 (5005)
- 详细的健康检查
- 开发友好的日志配置

## 🛠️ 故障排除

### 常见问题

1. **服务启动失败**
   ```bash
   # 检查日志
   ./scripts/manage.sh logs [service-name]
   
   # 检查端口占用
   netstat -tlnp | grep [port]
   ```

2. **数据库连接失败**
   ```bash
   # 检查MySQL服务状态
   docker-compose ps mysql
   
   # 检查数据库连接
   docker-compose exec mysql mysql -u root -p
   ```

3. **内存不足**
   - 重构后已移除资源限制，服务可以使用所有可用资源
   - 如需限制，可在 docker-compose.yaml 中重新添加 deploy 配置

### 日志位置

- **容器日志**: 使用 `docker-compose logs` 查看
- **应用日志**: 存储在 `fatcat_logs` 卷中
- **Nginx日志**: 存储在 `nginx_logs` 卷中
- **系统日志**: 在 `logs/` 目录中

## 🔄 版本升级

```bash
# 1. 备份数据
./scripts/backup.sh

# 2. 停止服务
./scripts/manage.sh stop

# 3. 更新代码/镜像
git pull
docker-compose pull

# 4. 重新构建应用
./scripts/manage.sh build

# 5. 启动服务
./scripts/manage.sh start
```

## 📞 技术支持

如果您遇到问题，请检查：

1. Docker 和 Docker Compose 版本是否符合要求
2. 系统资源是否充足（内存、磁盘空间）
3. 网络端口是否被占用
4. 配置文件语法是否正确

更多帮助请参考项目文档或提交 Issue。
